...
export async function generateInvoicePDF(data: InvoiceData): Promise<Uint8Array> {
  ...
  const sanitizeText = (text: string): string => {
    if (!text) return '';
    return text
      .replace(/\u202F/g, ' ')  // Espace insécable fine -> espace normale
      .replace(/\u00A0/g, ' ')  // Espace insécable -> espace normale
      .replace(/[\u2000-\u200B]/g, ' ') // Autres espaces Unicode -> espace normale
      ...
      .replace(/[^\x20-\x7E\xA0-\xFF]/g, ''); // Supprimer autres caractères non-WinAnsi
  };
  ...
  const drawRight = (text: string, x: number, y: number, size: number, font: PDFFont, color = COLOR.text) => {
    ...
    page.drawText(cleanText, { x: x - w, y, size, font, color });
  };
  ...
  // Company name
  page.drawText(sanitizeText('urstory.ch'), {
    x: MARGIN.left,
    y: leftY,
    size: TYPE.h1,
    font: bold,
  ...
  companyLines.forEach(line => {
    page.drawText(sanitizeText(line as string), {
      x: MARGIN.left,
      y: leftY,
      size: TYPE.body,
      font: regular,
  ...
  page.drawText(sanitizeText('Coordonnées de paiement'), {
  ...
  // IBAN
  if (ibanToDisplay) {
    page.drawText(sanitizeText('IBAN'), {
      x: MARGIN.left + SPACE.md,
      y: payY,
      size: TYPE.small,
      font: regular,
    ...
    payY -= SPACE.md;
    page.drawText(sanitizeText(ibanToDisplay), {
      x: MARGIN.left + SPACE.md,
      y: payY,
      size: TYPE.body,
      font: bold,
    ...
    if (settings.qr_iban) {
      page.drawText(sanitizeText(' (QR-IBAN)'), {
  ...
  // Titulaire
  page.drawText(sanitizeText('Titulaire:'), {
    x: MARGIN.left + SPACE.md,
    y: payY,
    size: TYPE.small,
    font: regular,
  ...
  page.drawText(sanitizeText(creditorName), {
  ...
  page.drawText(sanitizeText('Facturé à'), {
    x: MARGIN.left,
    y: clientY,
    size: TYPE.h1,
    font: bold,
  ...
  page.drawText(sanitizeText('Description'), { x: cols.desc + SPACE.sm, y: headerY, size: TYPE.body, font: bold, color: COLOR.white });
  page.drawText(sanitizeText('Qté'), { x: cols.qty + SPACE.sm, y: headerY, size: TYPE.body, font: bold, color: COLOR.white });
  page.drawText(sanitizeText('Unité'), { x: cols.unit + SPACE.xs, y: headerY, size: TYPE.body, font: bold, color: COLOR.white });
  page.drawText(sanitizeText('Prix unit.'), { x: cols.price + SPACE.xs, y: headerY, size: TYPE.body, font: bold, color: COLOR.white });
  ...
  data.items.forEach((item, index) => {
    ...
    // Unit
    page.drawText(sanitizeText('pc.'), { x: cols.unit + SPACE.xs, y: rowY, size: TYPE.body, font: regular, color: COLOR.textMuted });
  ...
  // Sous-total
  page.drawText(sanitizeText('Sous-total'), { x: totalsX, y: rowY - SPACE.sm, size: TYPE.body, font: regular, color: COLOR.text });
  ...
  page.drawText(sanitizeText(tvaLabel), { x: totalsX, y: rowY - SPACE.sm, size: TYPE.body, font: regular, color: COLOR.textMuted });
  ...
  page.drawText(sanitizeText('TOTAL'), {
    x: totalsX,
    y: rowY - SPACE.md,
    size: TYPE.h1,
    font: bold,
