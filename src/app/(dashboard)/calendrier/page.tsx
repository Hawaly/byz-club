...
function EntryForm({ form, setForm, onSubmit, onCancel, isSaving, submitLabel, displayedPiliers, getPillarColorDynamic, scripts }: {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-5">
      ...
      {/* Script lié au post */}
      <div>
        <label className={labelCls}>Script lié</label>
        <select
...
function EntryDetail({ entry, onEdit, onDelete, onStatusChange, onClose, displayedPiliers, getPillarColorDynamic, scripts }: {
  entry: CalendarEntry;
  onEdit: () => void;
  onDelete: () => void;
  ...
  const contentTypeLabels: Record<string, { label: string; icon: React.ComponentType<{className?:string}>; bg: string; color: string }> = {
    video:    { label: 'Vidéo',    icon: PlayCircle, bg: 'bg-blue-50',   color: 'text-blue-700'   },
    post:     { label: 'Post',     icon: FileText,   bg: 'bg-gray-100',  color: 'text-gray-700'   },
    carousel: { label: 'Carousel', icon: LayoutGrid, bg: 'bg-violet-50', color: 'text-violet-700' },
  };
  const ct = contentTypeLabels[entry.content_type] || { label: entry.content_type, icon: FileText, bg: 'bg-gray-100', color: 'text-gray-700' };
  const CtIcon = ct.icon;
  const pillarLabel = displayedPiliers.find(p => p.key === entry.pillar)?.label || entry.pillar;
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Thumbnail hero */}
      {entry.thumbnail_url && (
        <div className="relative -mx-6 -mt-6 h-52 overflow-hidden rounded-t-3xl">
          <img src={entry.thumbnail_url} alt="" className="w-full h-full object-cover" />
          <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent" />
          <div className="absolute bottom-4 left-6 right-6">
            <h2 className="text-xl font-black text-white drop-shadow-lg leading-tight">{entry.title}</h2>
            {entry.client && <p className="text-white/70 text-xs mt-1">{entry.client.name} · #{entry.id}</p>}
          </div>
        </div>
      )}

      {/* Titre + client (sans thumbnail) */}
      {!entry.thumbnail_url && (
        <div className="flex items-start justify-between gap-3 pb-1">
          <div className="flex-1">
            <h2 className="text-xl font-black text-gray-900 leading-tight">{entry.title}</h2>
            <p className="text-gray-400 text-xs mt-1">Post #{entry.id}</p>
          </div>
          {entry.client && (
            <span className="shrink-0 flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-full bg-blue-50 text-blue-700 border border-blue-200">
              <Users className="w-3 h-3" />{entry.client.name}
            </span>
          )}
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-gray-50 border border-gray-200 rounded-2xl p-4">
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1"><Calendar className="w-3 h-3" />Date de publication</p>
          <p className="text-sm font-bold text-gray-900">
            {new Date(entry.publish_date + 'T00:00:00').toLocaleDateString('fr-FR', { weekday: 'short', day: 'numeric', month: 'long', year: 'numeric' })}
          </p>
          {entry.publish_time && (
            <p className="text-xs text-gray-500 mt-1 flex items-center gap-1"><Clock className="w-3 h-3" />{entry.publish_time.slice(0, 5)}</p>
          )}
        </div>
        <div className="bg-gray-50 border border-gray-200 rounded-2xl p-4">
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5">Plateforme & Type</p>
          <p className={`text-sm font-bold flex items-center gap-1.5 ${plt?.color || 'text-gray-900'}`}><PlatIcon className="w-4 h-4" />{plt?.label || entry.platform}</p>
          <p className={`text-xs mt-1 flex items-center gap-1.5 ${ct.color}`}><CtIcon className="w-3 h-3" />{ct.label}</p>
        </div>
        <div className="bg-gray-50 border border-gray-200 rounded-2xl p-4">
          <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1.5 flex items-center gap-1"><ObjIcon className="w-3 h-3" />Objectif</p>
          <p className={`text-sm font-bold ${obj?.color || 'text-gray-900'}`}>{obj?.label || entry.objective}</p>
        </div>
        <div className="bg-gray-50 border border-gray-200 rounded-2xl p-4">
      ...
      {entry.persona && (
        <div className="flex items-center gap-3 bg-indigo-50 border border-indigo-200 rounded-2xl p-4">
          ...
          <div>
            ...
            <p className="text-sm font-bold text-indigo-900">{entry.persona}</p>
          </div>
        </div>
      )}
      ...
      {entry.strategic_intent && (
        <div className="bg-purple-50 border border-purple-200 rounded-2xl p-4">
          ...
          <p className="text-sm text-purple-900 italic leading-relaxed">"{entry.strategic_intent}"</p>
        </div>
      )}
      ...
      {entry.content_description && (
        <div className="bg-gray-50 border border-gray-200 rounded-2xl p-4">
          ...
          <p className="text-sm text-gray-700 leading-relaxed whitespace-pre-wrap">{entry.content_description}</p>
        </div>
      )}
      ...
      {entry.notes && (
        <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4">
          ...
          <p className="text-sm text-amber-900 leading-relaxed whitespace-pre-wrap">{entry.notes}</p>
        </div>
      )}
      ...
      <div className="flex flex-col gap-3">
        {entry.drive_link && (
          <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-2xl p-4">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-green-100 rounded-xl"><ExternalLink className="w-4 h-4 text-green-700" /></div>
              <div>
                <p className="text-[10px] font-black text-green-600 uppercase tracking-widest">Fichier Drive</p>
                <p className="text-xs text-green-800 font-medium mt-0.5 max-w-[200px] truncate">{entry.drive_link}</p>
              </div>
            </div>
            <a href={entry.drive_link} target="_blank" rel="noopener noreferrer"
              className="flex items-center gap-1.5 px-3 py-2 bg-green-600 hover:bg-green-700 text-white rounded-xl text-xs font-bold transition-all">
        ...
        {entry.thumbnail_url && (
          <div className="flex items-center justify-between bg-gray-50 border border-gray-200 rounded-2xl p-4">
            <div className="flex items-center gap-3">
              <div className="p-2 bg-gray-200 rounded-xl"><ImageIcon className="w-4 h-4 text-gray-600" /></div>
              <div>
                <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Thumbnail</p>
                <p className="text-xs text-gray-600 font-medium mt-0.5 max-w-[200px] truncate">{entry.thumbnail_url}</p>
              </div>
            </div>
            <a href={entry.thumbnail_url} target="_blank" rel="noopener noreferrer"
      ...
      {/* Script lié */}
      {entry.script_id && (() => {
        const linkedScript = scripts.find(s => s.id === entry.script_id);
        return linkedScript ? (
          <div className="bg-orange-50 border border-orange-200 rounded-2xl p-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-orange-100 rounded-xl"><FileText className="w-4 h-4 text-orange-600" /></div>
                <div>
                  <p className="text-[10px] font-black text-orange-600 uppercase tracking-widest">Script lié</p>
                  <p className="text-sm font-bold text-orange-900">{linkedScript.title}</p>
                </div>
              </div>
              <a href={`/scripts?highlight=${entry.script_id}`} onClick={onClose}
                className="flex items-center gap-1.5 px-3 py-2 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-xl text-xs font-bold shadow-sm hover:shadow-orange-200 transition-all">
      ...
      {/* Changer statut */}
      <div className="bg-gray-50 border border-gray-200 rounded-2xl p-4">
        <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-3">Changer le statut</p>
        <div className="flex flex-wrap gap-2">
          {Object.entries(STATUSES).map(([key, s]) => {
            const SIcon = s.icon;
            return (
              <button key={key} onClick={() => { onStatusChange(key); onClose(); }}
      ...
      <div className="flex gap-3 pt-1 border-t border-gray-100">
        <button onClick={onDelete}
          className="px-4 py-2.5 bg-red-50 text-red-600 border border-red-200 rounded-xl text-sm font-bold hover:bg-red-100 transition-all flex items-center gap-2">
          <Trash2 className="w-4 h-4" />Supprimer
        </button>
        <button onClick={onEdit}
          className="flex-1 flex items-center justify-center gap-2 px-6 py-2.5 bg-gradient-to-r from-purple-600 to-pink-600 text-white rounded-xl text-sm font-bold shadow hover:shadow-purple-200 transition-all">
