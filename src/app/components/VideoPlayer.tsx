...
export default function VideoPlayer({ videoId, title, thumbnailUrl, size = 'default' }: VideoPlayerProps) {
    ...
    const fetchThumbnail = async () => {
      try {
        ...
        if (data && data[0] && data[0].thumbnail_large) {
          // Replace _640 with something larger if needed, but thumbnail_large is usually 640
          setVimeoThumbnail(data[0].thumbnail_large.replace('_640', '_1280'));
        }
      } catch (error) {
  ...
  return (
    ...
    >
      ...
      {(thumbnailUrl || vimeoThumbnail) && !isPlaying && (
        <div className="absolute inset-0 z-20 transition-opacity duration-300">
          <img 
            src={thumbnailUrl || vimeoThumbnail || ''} 
            alt={title}
      ...
      <iframe
        ...
        title={title}
        className={`absolute inset-0 h-full w-full rounded-[var(--radius-lg)] transition-opacity duration-300 ${isPlaying ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
        allowFullScreen
      />
    </div>
  );
