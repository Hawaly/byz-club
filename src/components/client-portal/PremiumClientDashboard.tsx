...
export function PremiumClientDashboard({ user }: { user: User }) {
    ...
    const handleDownloadPdf = async (invoice: Invoice) => {
        ...
        try {
            ...
            if (pdfPath) {
                ...
                if (pdfPath.startsWith('/invoices/') || pdfPath.startsWith('/contracts/') || pdfPath.startsWith('/qr-bills/')) {
                ...
                // If it's a Supabase path (no leading slash), keep as-is
                else if (!pdfPath.startsWith('/') && !pdfPath.startsWith('http')) {
                    // This is a Supabase Storage path, we'll need to get a signed URL
                    // For now, try the API path
                    downloadPath = `/api/uploads/${pdfPath}`;
                }
            }
    ...
    const handleDownloadQr = async (invoice: Invoice) => {
        ...
        try {
            // If QR-bill path exists, use it directly
            if (invoice.qr_bill_path) {
                ...
                console.log('QR path from DB:', qrPath, '→', downloadPath);
                window.open(downloadPath, '_blank');
            } else {
                // Generate QR-bill via GET endpoint (returns PDF directly)
                console.log('Generating new QR-bill for invoice:', invoice.id);
                        ...
                        <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="fixed inset-4 md:inset-20 lg:inset-x-40 lg:inset-y-20 bg-white rounded-2xl shadow-2xl z-50 overflow-hidden flex flex-col max-w-2xl mx-auto">
                            ...
                            <div className="flex-1 overflow-y-auto p-6 space-y-6">
                                ...
                                <div className="flex gap-3">
                                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => { handleDownloadPdf(selectedInvoice); }} disabled={downloadingPdf === selectedInvoice.id} className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-xl font-semibold hover:shadow-lg transition-all disabled:opacity-50">
                                        {downloadingPdf === selectedInvoice.id ? <Loader2 className="w-5 h-5 animate-spin" /> : <Download className="w-5 h-5" />}
                                        Télécharger PDF
                                    </motion.button>
                                    <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => { handleDownloadQr(selectedInvoice); }} disabled={downloadingQr === selectedInvoice.id} className="flex-1 flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-xl font-semibold hover:shadow-lg transition-all disabled:opacity-50">
                                        {downloadingQr === selectedInvoice.id ? <Loader2 className="w-5 h-5 animate-spin" /> : <Download className="w-5 h-5" />}
                                        QR Code
                                    </motion.button>
                                </div>
                            </div>
                    ...
                    {activeTab === 'invoices' && (
                        <motion.div key="invoices" {...fadeInUp}>
                            <div className="mb-6">
                                ...
                                <p className="text-gray-600">Consultez et téléchargez vos factures en PDF et QR codes</p>
                            </div>
                            ...
                            {data.invoices.length === 0 ? (
                                ...
                                <div className="space-y-6">
                                    {/* Factures récentes - Section mise en avant */}
                                    {data.invoices.slice(0, 3).length > 0 && (
                                        <Card className="p-6 bg-gradient-to-br from-orange-50 to-white border-2 border-orange-200">
                                            <div className="flex items-center justify-between mb-4">
                                                <div>
                                                    ...
                                                    <p className="text-sm text-gray-600 mt-1">Accès rapide à vos dernières factures</p>
                                                </div>
                                            </div>
                                            <div className="grid gap-4 md:grid-cols-3">
                                                {data.invoices.slice(0, 3).map((invoice, i) => (
                                                    <motion.div key={invoice.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                                                        className="bg-white rounded-xl p-5 border-2 border-gray-100 hover:border-orange-300 hover:shadow-lg transition-all group">
                                                        <div className="flex items-start justify-between mb-3">
                                                            <div className="p-2.5 bg-purple-100 rounded-lg group-hover:bg-purple-200 transition-colors">
                                                                <FileText className="w-5 h-5 text-purple-600" />
                                                            </div>
                                                            <span className={`px-2.5 py-1 text-xs font-bold rounded-full ${invoice.status === 'payee' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>
                                                                {invoice.status === 'payee' ? '✓ Payée' : 'En attente'}
                                                            </span>
                                                        </div>
                                                        <h4 className="font-bold text-gray-900 mb-1">{invoice.invoice_number}</h4>
                                                        <p className="text-sm text-gray-600 mb-1">{new Date(invoice.issue_date).toLocaleDateString('fr-FR')}</p>
                                                        <p className="text-lg font-bold text-purple-600 mb-4">{invoice.total_ttc?.toLocaleString('fr-CH')} CHF</p>
                                                        <div className="flex gap-2">
                                                            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => handleDownloadPdf(invoice)} disabled={downloadingPdf === invoice.id}
                                                                className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 bg-gradient-to-r from-orange-500 to-red-500 text-white rounded-lg text-sm font-semibold hover:shadow-lg transition-all disabled:opacity-50">
                                                                {downloadingPdf === invoice.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />}
                                                                PDF
                                                            </motion.button>
                                                            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => handleDownloadQr(invoice)} disabled={downloadingQr === invoice.id}
                                                                className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg text-sm font-semibold hover:shadow-lg transition-all disabled:opacity-50">
                                                                {downloadingQr === invoice.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />}
                                                                QR
                                                            </motion.button>
                                                        </div>
                                                    </motion.div>
                                                    ...
                                                    <motion.tr key={invoice.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="hover:bg-orange-50/50 transition-colors group">
                                                        ...
                                                        <td className="px-6 py-4 whitespace-nowrap">
                                                            <div className="flex items-center gap-2">
                                                                ...
                                                                <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={() => handleDownloadQr(invoice)} disabled={downloadingQr === invoice.id}
                                                                    className="p-2.5 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-lg hover:shadow-lg transition-all disabled:opacity-50" title="Télécharger QR Code">
                                                                    {downloadingQr === invoice.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Download className="w-4 h-4" />}
                                                                </motion.button>
